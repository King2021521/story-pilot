...
```json
{
  "items": [
    {
      "name": "可直接用于正文的名称",
      "type": "character_name|city|location|organization|weapon|technique|item|place_name",
...
- 根据用户给定的题材、风格、数量、元素类型和世界规则约束生成。
- 避免同质化命名，同一批候选的音节、意象、结构不能高度重复。
- 避免时代感、文化感、科技层级错位。
- 名称要能直接用于小说正文，避免解释性占位名。
- 描述要给用户判断采纳价值，不要写空泛赞美。
- 理由要具体说明该候选如何服务冲突、悬念、人物、世界观或剧情推进。
